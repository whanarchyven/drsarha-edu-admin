...
export function InteractiveQuizGrid({
  ...
  const [interactiveQuizToDelete, setInteractiveQuizToDelete] = useState<
    string | null
  >(null);

  const handleDelete = async () => {
    if (!interactiveQuizToDelete) return;

    try {
      setIsDeleting(true);
      await interactiveQuizzesApi.delete(interactiveQuizToDelete);
      ...
      setIsDeleting(false);
      setInteractiveQuizToDelete(null);
    }
  };

  const openDeleteDialog = (id: string) => {
    setInteractiveQuizToDelete(id);
  ...
  const handleEdit = (id: string) => {
    router.push(`/knowledge/interactive-quizzes/${id}/edit`);
  };
