...
export const InsightQuestion = ({ question, getStats,setQuestionToDelete,hideBtns=false }: InsightQuestionProps) => {
            ...
            <CardHeader className="flex flex-col  gap-2 ">
                <div className="flex flex-row items-center gap-2 justify-between w-full">
                    <div className="flex flex-row items-center gap-4">
                        ...
                        <Badge variant="default">{question.response_type==="int" ? "Числовой" : "Варианты"}</Badge>
                    </div>
                    {!hideBtns && (
                        ...
                        <Button onClick={()=>{if(setQuestionToDelete) setQuestionToDelete(question)}} variant="destructive">
                            <Trash className="w-4 h-4" />
                        </Button>
                    </div>
                    )}
                </div>
                <Badge onClick={async()=>{await copyToClipboard(question.id??'');toast.success('ID скопирован в буфер обмена')}} className="w-fit cursor-pointer" variant="outline">ID: {question.id}</Badge>
            </CardHeader>
            <CardContent>
                <p>{question.prompt}</p>
                {question.response_type=='variants_multiple' && (
                    <div className="flex p-2 border rounded-md flex-col mt-4 gap-2">
                        ...
                        <div className="flex items-center flex-wrap gap-2">
                            {question?.response_variants?.map((variant) => (
                                <Badge className="w-fit" variant="outline">{variant}</Badge>
                            ))}
                        </div>
                    </div>
                )}
