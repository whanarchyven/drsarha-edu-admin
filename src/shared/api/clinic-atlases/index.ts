...
export const clinicAtlasesApi = {
  ...
  getById: async (id: string) => {
    const { data } = await axiosInstance.get<ClinicAtlas>(API.getClinicAtlasById(id));
    return data;
  },

  create: (data: FormData) => axiosInstance.post<ClinicAtlas>(API.createClinicAtlas, data, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  }),

  update: (id: string, data: FormData) => axiosInstance.put<ClinicAtlas>(API.updateClinicAtlas(id), data, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  }),
