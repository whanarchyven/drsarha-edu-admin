...
export default function MergeTable({initialStats}: {initialStats: Stat[]}) {
  ...
  const mappingExists = (sourceValue: string, targetValue: string) => {
    return mergeMappings.some((mapping) => mapping.source === sourceValue && mapping.target === targetValue)
  }
  ...
  const addMapping = (sourceValue: string, targetValue: string) => {
    ...
    if (!mappingExists(sourceValue, targetValue)) {
      setMergeMappings([...mergeMappings, { source: sourceValue, target: targetValue }])
    }
  }
  ...
  const removeMappingByValues = (sourceValue: string, targetValue: string) => {
    const newMappings = mergeMappings.filter(
      (mapping) => !(mapping.source === sourceValue && mapping.target === targetValue),
    )
  ...
  const handleMerge = async () => {
    ...
    try {
      const res=await pushConfigNamesMapPushConfigPost({
        mappings: mergeMappings.map((m) => ({ [String(m.source.split('(*)')[0]).toLowerCase()]: String(m.target.split('(*)')[0]) })),
      })
      console.log(res)
      setIsLoading(false)
      setMergeMappings([])
    ...
    <div className="space-y-6">
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {stats.map((stat) => (
              <TableRow key={stat.value} className={mergeSourceValue === stat.value ? "bg-muted/50" : ""}>
                <TableCell>{stat.value}</TableCell>
                <TableCell>{stat.count}</TableCell>
                <TableCell>
                  {!mergeSourceValue && (
                    <Button variant="outline" size="sm" onClick={() => startMerge(stat.value)}>
                  ...
                  {mergeSourceValue && mergeSourceValue !== stat.value && (
                    <Button
                      variant={mappingExists(mergeSourceValue, stat.value) ? "destructive" : "outline"}
                      size="sm"
                      onClick={() => {
                    ...
                    >
                      {mappingExists(mergeSourceValue, stat.value) ? "Удалить" : "Добавить"}
                    </Button>
                  )}
      ...
      {mergeSourceValue && (
        <div className="bg-muted p-4 rounded-md">
          <p className="font-medium mb-2">
            Выбран вариант для объединения: <span className="text-primary">{mergeSourceValue}</span>
          </p>
          <p className="text-sm text-muted-foreground mb-2">
            Нажмите "Добавить" у других вариантов, чтобы объединить их с выбранным, или "Отмена" для выхода из режима
      ...
      {mergeMappings.length > 0 && (
        <div className="border rounded-md p-4">
          ...
          <div className="space-y-2">
            {mergeMappings.map((mapping, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeMapping(index)}>
                  <X className="h-3 w-3" />
          ...
          <div className="mt-4">
            <Button onClick={handleMerge} variant="default">{isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Объединить"}</Button>
          </div>
        </div>
      )}
    </div>
