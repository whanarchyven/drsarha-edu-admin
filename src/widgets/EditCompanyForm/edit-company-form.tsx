...
import { PlusCircle, X, Plus, AlertCircle, Save, ArrowLeft, Loader2 } from "lucide-react"
...
export default function EditDashboardForm({ initialCompany }: { initialCompany: Company }) {
  ...
  const addDashboard = () => {
    ...
    setExpandedDashboards([...expandedDashboards, `dashboard-${newDashboardIndex}`])
  }

  const updateDashboard = (index: number, field: keyof Dashboard, value: string) => {
    if (!company) return
  ...
  const updateStat = (dashboardIndex: number, statIndex: number, field: keyof Stat, value: any) => {
    if (!company) return
  ...
  const removeScale = (dashboardIndex: number, statIndex: number, scaleIndex: number) => {
    if (!company) return

    const updatedDashboards = [...company.dashboards]
    updatedDashboards[dashboardIndex].stats[statIndex].scales.splice(scaleIndex, 1)
    setCompany({ ...company, dashboards: updatedDashboards })
  }
  ...
  const selectQuestion = (dashboardIndex: number, statIndex: number, questionId: string) => {
    if (!company) return
  ...
  const getQuestionText = (questionId: string) => {
    const question = questions.find((q:BaseInsightQuestionDto) => q.id === questionId)
  ...
  if (!company) {
    return (
      <div className="container mx-auto py-16 flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground mb-4">Запрашиваемая компания не найдена.</p>
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="mb-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href={`/companies/${company._id}`}>{company.name}</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Редактирование компании: {company.name}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Обновите основную информацию о компании</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Управляйте дашбордами для этой компании</CardDescription>
            </div>
            <Button type="button" onClick={addDashboard} variant="outline" size="sm">
          ...
          <CardContent>
            {company.dashboards.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-8 text-center border border-dashed rounded-lg">
                ...
                <Button type="button" onClick={addDashboard} variant="outline" size="sm" className="mt-4">
                  ...
                  >
                      ...
                      <AccordionTrigger className="py-2">
                        ...
                        {dashboard.stats.length > 0 && (
                          <Badge variant="outline" className="ml-2">
                            {dashboard.stats.length} стат{dashboard.stats.length !== 1 ? "" : "истика"}
                          </Badge>
                        )}
                      </AccordionTrigger>
                      <Button
                    ...
                    <AccordionContent>
                      <div className="space-y-4 p-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <Label htmlFor={`dashboard-name-${dashboardIndex}`}>Название дашборда</Label>
                            <Input
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => updateDashboard(dashboardIndex, "icon", e.target.value)}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <Button type="button" onClick={() => addStat(dashboardIndex)} variant="outline" size="sm">
                              <PlusCircle className="mr-2 h-4 w-4" />
                              Добавить статистику
                            </Button>
                          </div>
                          ...
                          {dashboard.stats.length === 0 ? (
                            <div className="flex flex-col items-center justify-center p-4 text-center border border-dashed rounded-lg">
                              <p className="text-muted-foreground text-sm">Статистика еще не добавлена</p>
                            </div>
                          ) : (
                              ...
                              {dashboard.stats.map((stat, statIndex) => (
                                <Card key={statIndex} className="relative">
                                  <Button
                                    ...
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => removeStat(dashboardIndex, statIndex)}
                                  ...
                                  <CardContent className="space-y-4 pb-2">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <div className="space-y-2">
                                        ...
                                        <Input
                                          ...
                                          value={stat.name}
                                          onChange={(e) =>
                                            updateStat(dashboardIndex, statIndex, "name", e.target.value)
                                          }
                                          placeholder="Введите название статистики"
                                        />
                                      </div>
                                      <div className="space-y-2">
                                        <Label htmlFor={`stat-type-${dashboardIndex}-${statIndex}`}>Тип</Label>
                                        <Select
                                          value={stat.type}
                                          onValueChange={(value) =>
                                            updateStat(dashboardIndex, statIndex, "type", value as DashboardType)
                                          }
                                        >
                                          <SelectTrigger id={`stat-type-${dashboardIndex}-${statIndex}`}>
                                            <SelectValue placeholder="Выберите тип" />
                                          </SelectTrigger>
                                          <SelectContent>
                                            <SelectItem value={DashboardType.LINE}>Линейный</SelectItem>
                                            <SelectItem value={DashboardType.BAR}>Столбчатый</SelectItem>
                                            <SelectItem value={DashboardType.PIE}>Круговой</SelectItem>
                                            <SelectItem value={DashboardType.AREA}>Область</SelectItem>
                                            <SelectItem value={DashboardType.TABLE}>Таблица</SelectItem>
                                          </SelectContent>
                                        </Select>
                                      </div>
                                    </div>
                                      ...
                                      <Dialog>
                                        <DialogTrigger asChild>
                                          <Button variant="outline" className="w-full justify-start">
                                            {stat.question_id ? getQuestionText(stat.question_id) : "Выберите вопрос"}
                                          </Button>
                                        </DialogTrigger>
                                        <DialogContent>
                                          <DialogHeader>
                                          ...
                                          <ScrollArea className="h-[300px] mt-4">
                                            <div className="space-y-2">
                                              {questions.map((question:BaseInsightQuestionDto) => (
                                                <Button
                                                  key={question.id}
                                                  variant={stat.question_id === question.id ? "default" : "outline"}
                                                  className="w-full justify-start text-left"
                                                  onClick={() => {
                                                      selectQuestion(dashboardIndex, statIndex, question.id??'')
                                                  }}
                                                >
                                                  {question.title}
                                                </Button>
                                    ...
                                    <div className="space-y-2">
                                      <Label htmlFor={`scale-all-${dashboardIndex}-${statIndex}`}>Общий масштаб</Label>
                                      <Input
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between">
                                        ...
                                        <Button
                                          type="button"
                                          onClick={() => addScale(dashboardIndex, statIndex)}
                                          variant="outline"
                                          size="sm"
                                        >
                                          <PlusCircle className="mr-2 h-4 w-4" />
                                      ...
                                      {stat.scales.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center p-4 text-center border border-dashed rounded-lg">
                                          <p className="text-muted-foreground text-sm">Масштабы еще не добавлены</p>
                                        </div>
                                      ) : (
                                        <div className="space-y-3">
                                          {stat.scales.map((scale, scaleIndex) => (
                                            <div
                                              key={scaleIndex}
                                            ...
                                            >
                                              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 flex-1">
                                                ...
                                                <div className="space-y-1">
                                                  ...
                                                  <div className="flex gap-2">
                                                    <Button
                                                      type="button"
                                                      variant={scale.type === "linear" ? "default" : "outline"}
                                                      size="sm"
                                                    ...
                                                    <Button
                                                      type="button"
                                                      variant={scale.type === "multiple" ? "default" : "outline"}
                                                      size="sm"
                                                      className="flex-1 h-8"
                                                      onClick={() =>
                                                        updateScale(
                                              ...
                                              <Button
                                                ...
                                                variant="ghost"
                                                size="sm"
                                                onClick={() => removeScale(dashboardIndex, statIndex, scaleIndex)}
